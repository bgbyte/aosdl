...
def lookup_ga_build():
    ...
    print("Getting AOS version information...","\nIf you know which build you want, skip the following step:")
    while True:
        ga_prompt_fam = input("Optionally, enter the switch family name if you want to lookup the GA build # (e.g., shasta) [skip]: ").strip().lower() or None
        if not ga_prompt_fam:
            print("Skipping GA build lookup...")
            break
        ga_prompt_ver = input("Provide the AOS version & Release for the build you want to lookup (e.g., 8.10R02) [skip]: ").strip().upper() or None
        if ga_prompt_ver:
            print(get_ga_build(ga_prompt_ver, ga_prompt_fam))
        else:
